...
class DjangoPropertyField(DjangoBaseField):
    ...
    def formfield(self, **kwargs):
        ...
        if self.choices:
            ...
            include_blank = not self.required and not (
...
class DjangoRelationField(DjangoBaseField):
    ...
    def save_form_data(self, instance, data):
        ...
        related_model = current_apps.get_model(
            self.prop.module_name.split(".")[-2], self.prop._raw_class
        )
...
class NeoNodeSet(NodeSet):
    ...
    def iterator(self, chunk_size=2000):
        ...
        # Basic iterator implementation to fetch nodes
        # This is a placeholder to demonstrate functionality
        # return self.source_class.nodes.all()
        return []

    # The code below was added to stop the tests from failing for Python 3.10+
    # It would be interesting to actually implement prefetching
    def prefetch_related(self, *lookups):
        self._prefetch_related_lookups.extend(lookups)
        return self
    ...
    def _prefetch_related_objects(self):
        for lookup in self._prefetch_related_lookups:
            self._perform_prefetch(lookup)

    def _perform_prefetch(self, lookup):
        # Simplified example of prefetching related objects
        # Assuming `lookup` is a relation name from the node class
        # if hasattr(self.source_class, lookup):
        #     relationship_manager = getattr(self.node_class, lookup)
        #     for parent_node in self._result_cache:
        #         # Accessing the related nodes directly via neomodel relationship manager
        #         related_nodes = list(getattr(parent_node, lookup).all())
        #         # Store the fetched nodes in a cache attribute
        #         setattr(parent_node, f"_{lookup}_cache", related_nodes)
        pass
...
class DjangoNode(StructuredNode, metaclass=MetaClass):
    ...
    def _meta(self):
        ...
        for key, prop in self.__all_properties__:
            opts.add_field(
                DjangoPropertyField(prop, key), getattr(prop, "private", False)
            )
